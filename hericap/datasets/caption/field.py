...
class ImageField:
    def __init__(
        ...
        **kwargs,):
        ...
        self.use_hdf5_feat = use_hdf5_feat # False
        self.use_reg_feat = use_reg_feat # TURE
        self.use_gri_feat = use_gri_feat # TURE
    ...
    def init_hdf5_feat(self):
        ...
        # hdf5_path文件来源于 train_caption.extract_vis_features()
        # 无增强image，经过 Swin + Project + Deformable DETR，提取的region特征和grid特征
        # 包含三个数据集 ['image_ids'] ['gri_feat']['gri_mask'] ['reg_feat' ] ['reg_mask']
        # ['image_ids'] = 图片名称int，小到大..... 4,55,57,99
        with h5py.File(self.hdf5_path, 'r') as f:
            self.image_ids = f['image_ids'][:len(f['image_ids'])]  # HDF5 数据集对象 -> NumPy 数组
            self.img_id2idx = {img_id: img_idx for img_idx, img_id in enumerate(self.image_ids)} #  图片名称int:次序

    # 如果从hdf5读，直接获取gri和reg特征
    # 不从hdf5读，获取transform变换后image
    def preprocess(self, path, image_id=None):
        ...
        if self.use_hdf5_feat:
            ...
            # print("len(outputs['gri_feat'])  in /gemini/code/grit/datasets/caption/field.py ", outputs['gri_feat'].shape)
            return outputs
        else:

...
from models.caption.glob_net import GlobFeatureNetwork
from models.caption.cap_generator import CaptionGenerator


class Transformer(BaseCaptioner):
    # detector : Swin + Project + Deformable DETR
    def __init__(self, detector, config=None):
        ...
        self.glob_net = GlobFeatureNetwork(
            n_layers=config.model.glob_net.n_layers,
            d_in=config.model.glob_feat_dim,
            dropout=config.model.dropout,
        )
        ...
        self.use_glob_feat = config.model.use_glob_feat
        ...
        if self.use_reg_feat:
            ...
            self.register_state('reg_point', None)
        
        if self.use_glob_feat:
            self.register_state('glo_feat', None)   
            self.register_state('glo_mask', None)
    ...
    def forward(self, images, seq, use_beam_search=False,
                max_len=20,
                eos_idx=3,
                beam_size=5,
                out_size=1,
        ...
        if not use_beam_search:  #  False
            # Region Feature (Swin + Project + Deformable DETR)
            ...
            if self.config.model.use_gri_feat:
                # [b, 3, (h w), 512]  d_model   <---  [16, 60, 1024]   grid_feat_dim
            ...
            # Global Feature
            if self.config.model.use_glob_feat:
                # [b, 3, (h w), 512]  d_model   <---   [16, 1024, 6, 10]   glob_feat_dim
                glo_feat , vis_inputs['glo_mask'] = self.glob_net(vis_inputs['glo_feat'], vis_inputs['glo_mask'])
                # [b, (h w), 512]
                vis_inputs['glo_feat'] = glo_feat[:, -1]



            # Cap_generator
            dec_output, _ = self.cap_generator(seq, vis_inputs)  

            # assert False,"in transfomer.py" 
            return dec_output
            ...
            with self.statefulness(batch_size):
                ...
                weights_all_timestep=[]

                for timestep in range(max_len):
                    images, outputs, atten_weights_list= self.iter(
                        timestep=timestep,
                        samples=images,
                        outputs=outputs,
                    ...
                    weights_all_timestep.append(atten_weights_list) 
                    #  atten_weights_list: 
            ...
            if return_probs:
                ...
                return outputs, log_probs, weights_all_timestep
    ...
    def step(self, timestep, prev_output, samples, seq, mode='teacher_forcing', **kwargs):
        ...
        if mode == 'teacher_forcing':
            ...
            if timestep == 0:
                ...
                if self.config.model.use_reg_feat:
                    ...
                    self.reg_point = vis_inputs['reg_point']

                if self.config.model.use_glob_feat:
                    self.glo_feat, self.glo_mask  = self.glob_net(vis_inputs['glo_feat'], vis_inputs['glo_mask'])
                    self.glo_feat = self.glo_feat[:, -1]

                # If t = 0, enc_output = [B, N, D], init_tokens = [B, 1]
                # Else t > 0, enc_output = [BB, N, D], it = prev_output (t-1) = [BB, 1]
                _feat = getattr(self, 'gri_feat', self.reg_feat)  # vis_inputs.shape  [batch, (h w), 512] [batch, 150, 512] [batch, (h w), 512]
        ...
        if self.config.model.use_reg_feat:
            ...
            vis_inputs['reg_point'] = self.reg_point
        
        if self.config.model.use_glob_feat:
            vis_inputs['glo_feat'] = self.glo_feat
            vis_inputs['glo_mask'] = self.glo_mask

        # print("in transformer.py")
        # print("vis_inputs['gri_feat'].shape:",vis_inputs['gri_feat'].shape)  #  h, w = 6, 8(10/9/7)
        # print("in transformer.py")
        # print("vis_inputs['glo_feat'].shape:",vis_inputs['glo_feat'].shape)  #  h, w = 6, 10
    ...
    def iter(self, timestep, samples, outputs, return_probs, batch_size, beam_size=5, eos_idx=3, **kwargs):
        ...
        word_logprob, atten_weights_list = self.step(timestep, self.selected_words, samples, None, mode='feedback', **kwargs)
        ...
        return samples, outputs, atten_weights_list

...
class Attention_W(nn.Module):
    ...
    def forward(self, q, k, v, attention_mask=None):
        ...
        if self.n_memories > 0: # memory bank
        ...
        p_attn1 = self.dropout(p_attn)

        # [b h nq nk] * [b h nk dv] = [b h nq dv] -> [b nq h dv] -> [b nq h*dv]
        out = rearrange(torch.matmul(p_attn1, v), 'b h nq dv -> b nq (h dv)')

        out = self.fc_o(out)  # (b_s, nq, d_model)
        return out , p_attn # [b h nq nk]
...
class MultiHeadAttention_W(Module):

    def __init__(self, d_model, n_heads, dropout=.1, n_memories=0, can_be_stateful=False):
        super().__init__()

        self.attention = Attention_W(d_model=d_model, n_heads=n_heads, dropout=dropout, n_memories=n_memories)
        self.dropout = nn.Dropout(p=dropout)
        self.layer_norm = nn.LayerNorm(d_model)

        self.can_be_stateful = can_be_stateful # False
    ...
    def forward(self, queries, keys, values, attention_mask=None):
        ...
        out, p_attn  = self.attention(queries, keys, values, attention_mask)

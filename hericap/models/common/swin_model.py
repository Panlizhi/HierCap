...
class PatchMerging(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        B, L, C = x.shape
        assert L == H * W, "swin input feature has wrong size"
        x = x.view(B, H, W, C)
    ...
    def forward(self, x, H, W):
        ...
        for n_blk, blk in enumerate(self.blocks):
            ...
            if self.use_checkpoint: # false
                ...
                x = blk(x, attn_mask)   #  B H*W C  --->   B H'*W' C

        # reduce the number of tokens and increase the channel dim by a factor of 2
        # swin_stage1 + merge / swin_stage2 + merge / swin_stage3 + merge / swin_stage4 + merge 
        # swin_stage4_output = (H/32, W/32, 8C) --> (H/64, W/64, 8C*4)--> (H/64, W/64, 1024)
        if self.downsample is not None:
            x_down = self.downsample(x, H, W)   # [B H'*W' C] --->   #  [B, H'/2*W'/2, 2*C]
            Wh, Ww = (H + 1) // 2, (W + 1) // 2
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.proj(x)  # B, C=96, Wh= origin_h / patch_size , Ww = origin_w / patch_size    after pading
...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.frozen_stages = frozen_stages  # 2
        ...
        self.patch_embed = PatchEmbed(
            ...
            norm_layer=norm_layer if self.patch_norm else None)  # LayerNorm = True
        ...
        for i_layer in range(self.num_layers):  # has 4 stages/layers
            layer = BasicLayer(
                dim=int(embed_dim * 2**i_layer), # C,2C,4C,8C
    ...
    def forward(self, x):
        ...
        # patch embedding（patch partition + liner project）
        x = self.patch_embed(x)  #[B, C=128, Wh=ori_H/4, Ww=ori_W/4]

        Wh, Ww = x.size(2), x.size(3)
        x = x.flatten(2).transpose(1, 2) # torch.Size([16, 15360, 128])
        x = self.pos_drop(x)  # [B, Wh*Ww, C ]  torch.Size([16, 15360, 128])  
        
        outs = []
        for stage in range(self.num_layers):
            layer = self.layers[stage]
            #                                              = [B,Wh*Ww,C]   Wh  Ww
            #[B,Wh*Ww,C] Wh Ww  [B,Wh/2*Ww/2,2C] Wh/2 Ww/2 =
            x_out, H, W, x, Wh, Ww = layer(x, Wh, Ww)    

            # Aggregate intermediate outputs
 
            if stage > 0:
                patch_out = x_out.view(B, H, W, -1).permute(0, 3, 1, 2) # [B, C, Wh, Ww]
                outs.append(patch_out)
                # print("patch_out in :", patch_out.shape)
                # print("outs in :", len(outs))
    ...
    def train(self, mode=True):  # 冻结前2层
...
def swin_base_win7_384(pretrained=None, pos_dim=1024, **kwargs):
    ...
    if pretrained is None or pretrained == 'none':
        # print("------swin pretrained---------:",pretrained)
        return model, 1024

    # pretrain imagenet or other

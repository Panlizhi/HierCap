...
# add /tmp into sys.path when offline training.
# sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '/tmp/hericap')))
...
class DetectionModule(nn.Module):
    ...
    def bbox_refine(self, bbox_embed, output, reference_points):
        ...
        if bbox_embed is not None:
            tmp = bbox_embed(output) # [bs, 300, 256] -> [bs, 300, 4(xywh)]
    ...
    def __init__(
        ...
        self.return_intermediate = return_intermediate_dec   # True
    ...
    def prepare_od_inputs(self, srcs, masks):
        ...
        for lvl, (src, mask) in enumerate(zip(srcs, masks)):
            ...
            src = src.flatten(2).transpose(1, 2)  # [B, H1 *W1, 512 ]
            mask = mask.flatten(1)                # [B, H1 *W1 ]
            src_flatten.append(src)
            mask_flatten.append(mask)
        src_flatten = torch.cat(src_flatten, 1)  # [B, H1*W1 + h2*w2 +...+h4*w4, C] has 4 levels        multi-level scale feature， 
        ...
        return {
            'tgt': query_tgt,     
            'src': src_flatten,  # [B, H1*W1 + h2*w2 +...+h4*w4, C] has 4 levels
    ...
    def forward(self, srcs, masks):
        ...
        for lid, layer in enumerate(self.decoder_layers):
            ...
            # print("od_inputs['tgt'].shape",od_inputs['tgt'].shape)
        ...
        if self.return_intermediate:
            ...
            #  intermediate_all[0].shape   torch.Size([16, 150, 512]）
        ...
        # hs:  7   [[B, num_queries, C],  [B, num_queries, C],......]
        # inter_references:  7  [[B, queries, 4],[B, queries, 4]........]
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward(self,
        ...
        tgt2 = self.cross_attn(self.with_pos_embed(tgt, query_pos), reference_points, src, src_spatial_shapes,
                               src_level_start_index, src_padding_mask)   #  tgt2.shape = B, h*w,  d_model
